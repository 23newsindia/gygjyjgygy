...
protected function get_filtered_products($settings) {
    ...
    $args = [
     ...
     // Handle category filtering - IMPROVED VERSION
    if (!empty($settings['category'])) {
       error_log('Selected Category ID: ' . $settings['category']);
        // Get all child categories (including grandchildren)
        $category_terms = get_terms([
    'taxonomy' => 'product_cat',
    'child_of' => $settings['category'],
    'fields' => 'ids',
    'hide_empty' => false
]);
error_log('Child Terms Found: ' . print_r($category_terms, true));
        
        // Include parent category
        $category_terms[] = $settings['category'];
        
        // Remove any errors and ensure unique IDs
        $category_terms = array_unique(array_filter($category_terms, 'is_numeric'));
        
        // Debug the category terms being used
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('Category Terms Used: ' . print_r($category_terms, true));
        }
...
error_log('Final Tax Query: ' . print_r($args['tax_query'], true));
    }
    ...
    if (!empty($settings['discount_rule']) && class_exists('CTD_DB')) {
        ...
        if ($rule) {
            ...
            $all_rule_cats = array_unique(array_merge($rule_categories, $rule_cat_children));
            ...
            // If category is selected, find intersection
            if (!empty($settings['category'])) {
                $selected_cat_children = get_term_children($settings['category'], 'product_cat');
                $selected_cat_tree = array_unique(array_merge([$settings['category']], $selected_cat_children));
                $valid_cats = array_intersect($selected_cat_tree, $all_rule_cats);
